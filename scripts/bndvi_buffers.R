...
merge_rasters <- function(pattern) {
  paths <- list.files('spatial_data/bndvi', pattern=pattern, full.names=T)
...
for (i in years) {
  ...
  merged <- merge_rasters(i)
  merged@data@names<-i
  writeRaster(merged, paste0('spatial_data/bndvi/', i, '.asc'), format='ascii', overwrite=T)
  rm(merged)
}
...
for (i in years) {
  ...
  writeRaster(r, paste0('spatial_data/bndvi/bndvi_', i, '.asc'))
  rm(r)
                      ...
                      .inorder=T) %dopar% {
                        ...
                        if (landuses[i,'shannon_index']>=median_shannon) {
                          ...
                          if (dataset_sf$Ano_CS[i]==2016) {
                            stack2016 <- stack(raster('spatial_data/bndvi/bndvi_Mean_2016.asc'),
                                               raster('spatial_data/bndvi/bndvi_StdDev_2016.asc'))
                            values <- na.omit(as.data.frame(raster::extract(stack2016, buffer)))
                            rm(stack2016)
                          }
                          else if (dataset_sf$Ano_CS[i]==2017) {
                            stack2017 <- stack(list(raster('spatial_data/bndvi/Mean_2017.asc'),
                                                    raster('spatial_data/bndvi/StdDev_2017.asc')))
                            values <- na.omit(as.data.frame(raster::extract(stack2017, buffer)))
                            rm(stack2017)
                          }
                          else if (dataset_sf$Ano_CS[i]==2018) {
                            stack2018 <- stack(list(raster('spatial_data/bndvi/Mean_2018.asc'),
                                                    raster('spatial_data/bndvi/StdDev_2018.asc')))
                          ...
                          if (dataset_sf$Ano_CS[i]==2016) {
                            stack2016 <- stack(list(raster('spatial_data/bndvi/Mean_2016.asc'),
                                                    raster('spatial_data/bndvi/StdDev_2016.asc')))
                            values <- na.omit(as.data.frame(raster::extract(stack2016, buffer)))
                            rm(stack2016)
                          }
                          else if (dataset_sf$Ano_CS[i]==2017) {
...
colnames(as.data.frame(bndvi)) <- c('Mean_BNDVI', 'StdDev_BNDVI')
